...
uni_axis = cross(rev_in_axis, rev_out_axis);

leg_knee_coordinates = leg_middle_coordinates + (sphere_radius - end_effector_radius)/2 * uni_axis;

leg_knee2_coordinates = leg_inner_end_coordinates + leg_axes/4;

for i = 4:-1:1
    mechanism(i) = struct('matrix', ...
        [mmc_prismatic_joint(leg_knee2_coordinates(:, i) - leg_inner_end_coordinates(:, i)), ...
         mmc_revolute_joint(rev_in_axis(:, i), leg_knee2_coordinates(:, i)), ...
         mmc_universal_joint(uni_axis(:, i), leg_knee_coordinates(:, i)), ...
         mmc_revolute_joint(rev_out_axis(:, i), leg_outer_end_coordinates(:, i))]);
