function [screw_matrix] = mmc_planar_joint(missing_axis)

if ~isequal(size(missing_axis), [3 1])
    error('mmc:mmc_planar_joint', 'Axis vector must be a column of 3 elements.');
end

missing_axis_norm = sqrt(sum(missing_axis.^2));
if missing_axis_norm < 1e-6
    error('mmc:mmc_planar_joint', 'Axis vector norm is too small.');
end
...
screw_matrix = [mmc_prismatic_joint(axis1), ...
                mmc_prismatic_joint(axis2)];
